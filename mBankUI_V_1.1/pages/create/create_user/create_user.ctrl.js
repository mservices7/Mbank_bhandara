...
scotchApp.controller('userController', function ($rootScope, $interval, $timeout, $scope, $http, $routeParams, $location, $filter, $cookieStore) {
   ...
   // alert('external_login_id for user' + external_login_id);
    ...
    //alert('external_agent_id for agent + ' + external_agent_id);
        ...
        var request = $http({
            ...
            data: {
               // external_agent_id: String(login_id),
                external_agent_id: String(external_agent_id),
            ...
            $http.get(linkglobal + '/agents?$filter=bank_id eq ' + imageIDData + ' and external_agent_id+eq+' + "'" + external_agent_id + "'").success(function (response) {
                var amt = 0;
                var agent = response;
